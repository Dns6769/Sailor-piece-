...
local function IsSmartMatch(npcName, targetMobType)
    ...
    -- If target is a boss type, only match if npc name also contains "boss"
    -- This prevents "Thief" mobs matching a "ThiefBoss" quest target
    if t:find("boss") then
        return n == t
    end

    -- If npc name contains "boss" but target does not, skip
    -- This prevents ThiefBoss being farmed when quest wants Thief
    if n:find("boss") and not t:find("boss") then
        return false
    end
...
local function EquipWeapon()
    ...
    -- If SwitchWeapon rotation is enabled, use rotation logic
    ...
    -- Always fallback: equip whatever tool is available
    -- Prioritize Sword > Melee > any Tool
    -- This ensures M1 always fires with something equipped
    local alreadyHeld = char:FindFirstChildOfClass("Tool")
    if alreadyHeld then return end -- already holding something, fine

    local priority = {"Sword", "Katana", "Melee", "Combat"}
    ...
    -- Try any Sword/Melee type from module
    for _, tool in ipairs(backpack:GetChildren()) do
        if tool:IsA("Tool") then
            local t = GetToolTypeFromModule(tool.Name)
            if t == "Sword" or t == "Melee" then
                hum:EquipTool(tool); return
            end
        end
    end

    -- Last resort: equip first tool found
    local anyTool = backpack:FindFirstChildOfClass("Tool")
    if anyTool then hum:EquipTool(anyTool) end
end
...
local function GetQuestMobTarget()
    -- Read the actual quest requirement from the live quest UI text
    -- as a fallback when module data is mismatched
    local ok, result = pcall(function()
        local info = PGui.QuestUI.Quest.Quest.Holder.Content.QuestInfo
        local desc = info.QuestDescription.Text or ""
        -- Description usually says "Defeat X [Lv.Y]" — extract the mob name
        local mobName = desc:match("Defeat%s+(.-)%s+%[") or desc:match("Defeat%s+(.-)%s*$")
        return mobName
    end)
...
local function UpdateQuest()
    ...
    -- Force re-accept if:
    -- 1. QuestNPC changed (better quest available for new level)
    -- 2. Quest UI is not visible (quest not active)
    -- 3. Player leveled up since last check (may have unlocked better quest)
    local levelChanged = currentLevel ~= _lastCheckedLevel
    local npcChanged = Shared.QuestNPC ~= targetNPC
    local questNotActive = not questUI.Visible

    _lastCheckedLevel = currentLevel

    if npcChanged or questNotActive or (levelChanged and npcChanged) then

        -- Abandon current quest first
        ...
        -- Small delay before accepting new quest
        task.wait(0.3)
...
local function GetLevelFarmTarget()
    ...
    -- PRIMARY: Read mob target from live quest UI description
    -- This is the most reliable source — reflects the ACTUAL active quest
    -- regardless of what the module data says
    local targetMobType = nil

    pcall(function()
        local info = PGui.QuestUI.Quest.Quest.Holder.Content.QuestInfo
        local title = info.QuestTitle.QuestTitle.Text or ""
        local desc = info.QuestDescription.Text or ""

        -- Try to extract from description: "Defeat X [Lv.Y]" or "Defeat X (qty)"
        local fromDesc = desc:match("Defeat%s+(.-)%s+%[")
                      or desc:match("Defeat%s+(.-)%s*%(")
                      or desc:match("Defeat%s+(.-)%s*$")

        -- Try title as fallback: "Defeat ThiefBoss" etc
        local fromTitle = title:match("Defeat%s+(.-)%s*$")

        targetMobType = fromDesc or fromTitle
    end)

    -- FALLBACK: Use module data if UI read failed
    if not targetMobType or targetMobType == "" then
        local questData = Modules.Quests.RepeatableQuests[Shared.QuestNPC]
        if questData and questData.requirements and questData.requirements[1] then
            targetMobType = questData.requirements[1].npcType
        end
    end

    if not targetMobType or targetMobType == "" then return nil end

    local cleanTarget = targetMobType:gsub("%s+", ""):lower()
    local isBossQuest = cleanTarget:find("boss") ~= nil

    -- For boss quests: find the boss directly — do NOT use cluster logic
    -- Cluster logic picks the biggest group which is always regular mobs
    if isBossQuest then
        local char = GetCharacter()
        local root = char and char:FindFirstChild("HumanoidRootPart")
        local bestBoss = nil
        ...
        for _, npc in pairs(PATH.Mobs:GetChildren()) do
            if npc:IsA("Model") and npc:FindFirstChildOfClass("Humanoid") then
                local cleanNPC = npc.Name:gsub("%d+$", ""):gsub("%s+", ""):lower()
                -- Exact match only for boss targets
                if cleanNPC == cleanTarget and IsValidTarget(npc) then
                    local dist = root and (root.Position - npc:GetPivot().Position).Magnitude or 0
        ...
        if bestBoss then
            local island = GetNearestIsland(bestBoss:GetPivot().Position, bestBoss.Name)
            return bestBoss, island, "Boss"
        end
        -- Boss not spawned yet — return nil so farm waits/summons
        return nil
    end

    -- For regular mob quests: use cluster logic but strictly exclude boss variants
    local matches = {}
    for _, npc in pairs(PATH.Mobs:GetChildren()) do
        if npc:IsA("Model") and npc:FindFirstChildOfClass("Humanoid") then
            local cleanNPC = npc.Name:gsub("%d+$", ""):gsub("%s+", ""):lower()
            -- Skip any boss NPCs when quest is for regular mobs
            if not cleanNPC:find("boss") then
                local isMatch = cleanNPC == cleanTarget
                             or (cleanNPC:find(cleanTarget, 1, true) == 1
                                 and math.abs(#cleanNPC - #cleanTarget) <= 2)
                if isMatch then
        ...
        if isInTower then
            ...
            if bestTarget then
                ...
                if tick() - Shared.LastM1 >= m1Delay then
                    ...
                    local firePos = targetHRP and targetHRP.Position or targetPos
                    Remotes.M1:FireServer(firePos)
                    Shared.LastM1 = tick()
                end
            else
                Shared.Target = nil
