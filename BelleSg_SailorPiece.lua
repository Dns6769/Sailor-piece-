...
local function GetLevelFarmTarget()
    ...
    -- Build match list — exact matching only, no partial boss/mob confusion
    local matches = {}
    ...
    for _, npc in pairs(PATH.Mobs:GetChildren()) do
        if npc:IsA("Model") and npc:FindFirstChildOfClass("Humanoid") then
            ...
            -- Strict: exact match OR npc starts with target AND lengths are close
            -- Prevents Thief matching ThiefBoss and vice versa
            local isMatch = false
            if cleanNPC == cleanTarget then
                isMatch = true
            elseif cleanTarget:find("boss") then
                -- Quest wants a boss — only match if npc name also has "boss"
                isMatch = cleanNPC == cleanTarget
            elseif not cleanNPC:find("boss") then
                -- Quest wants a regular mob — skip any boss NPCs
                isMatch = cleanNPC:find(cleanTarget, 1, true) == 1
                          and math.abs(#cleanNPC - #cleanTarget) <= 2
            end

            if isMatch then
                local displayName = npc.Name:gsub("%d+$", "")
                matches[displayName] = true
            end
        end
    end
