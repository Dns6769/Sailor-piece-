if getgenv().BelleSg_Running then
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("Belle.sg â€¢ Session: %s â€¢ %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "Belle.sg â€¢ " .. os.date("%x %X") }
        }}
    }
            ...
            if url ~= "" and url:find("discord.com/api/webhooks/") then
                local payload = {
                    ["embeds"] = {{
                        ...
                        ["footer"] = { ["text"] = "Belle.sg | Sailor Piece" }
                    }}
                }
                task.spawn(function()
                    pcall(function()
...
local function IsSmartMatch(npcName, targetMobType)
    -- Strip trailing digits (e.g. "Monkey1" -> "Monkey")
    ...
    -- Only allow prefix match if the match is COMPLETE word boundary
    -- e.g. quest "monkey" must NOT match "monkeyboss"
    -- Check: after the prefix, next char must not be a letter (prevents boss crossover)
    local function strictPrefix(haystack, needle)
        if haystack:sub(1, #needle) == needle then
            local rest = haystack:sub(#needle + 1)
            -- if nothing follows, or what follows is not a letter, it's a valid match
            return rest == "" or not rest:sub(1,1):match("%a")
        end
        return false
    end

    if strictPrefix(n, t) then return true end
    if strictPrefix(t, n) then return true end
...
local function EnsureQuestSettings()
    -- Always fire unconditionally — server is idempotent, safe to repeat
    -- pcall wraps UI path check so broken hierarchy doesn't break quest loop
    pcall(function()
    ...
    -- Fallback: fire unconditionally so quest repeat always enabled
    pcall(function() Remotes.SettingsToggle:FireServer("EnableQuestRepeat", true) end)
    pcall(function() Remotes.SettingsToggle:FireServer("AutoQuestRepeat", true) end)
end
...
local function UpdateQuest()
    ...
    -- Already on correct quest and UI visible — nothing to do
    if Shared.QuestNPC == targetNPC and questUI.Visible then return end

    -- Step 1: abandon current quest cleanly
    if questUI.Visible then
        Remotes.QuestAbandon:FireServer("repeatable")
        local t = 0
        repeat task.wait(0.2); t = t + 1 until not questUI.Visible or t >= 20
        task.wait(0.5) -- extra settle time before accepting new quest
    end

    -- Step 2: accept new quest — retry up to 5 times with 1s gap
    Shared.QuestNPC = "" -- reset so we know if accept fails
    for attempt = 1, 5 do
        Remotes.QuestAccept:FireServer(targetNPC)
        local t = 0
        repeat task.wait(0.2); t = t + 1 until questUI.Visible or t >= 15
        if questUI.Visible then
    ...
    -- Step 3: if still no quest UI, try teleport to quest NPC & accept again
    if not questUI.Visible then
        pcall(function()
            local npcModel = workspace.ServiceNPCs:FindFirstChild(targetNPC)
                          or workspace:FindFirstChild("ServiceNPCs")
                          and workspace.ServiceNPCs:FindFirstChild(targetNPC)
            if npcModel then
                local hrp = npcModel:FindFirstChild("HumanoidRootPart")
                local myRoot = GetCharacter() and GetCharacter():FindFirstChild("HumanoidRootPart")
                if hrp and myRoot then
                    myRoot.CFrame = hrp.CFrame * CFrame.new(0, 0, 5)
                    task.wait(0.5)
                    Remotes.QuestAccept:FireServer(targetNPC)
                    task.wait(1)
                    if questUI.Visible then
...
local function GetLevelFarmTarget()
    ...
    -- If quest still not visible after UpdateQuest, wait a tick and check once more
    -- before giving up — avoids teleporting to mobs without active quest
    ...
    -- If we already have a valid living target that matches quest — keep it
    -- This prevents switching mid-kill
    if Shared.Target and Shared.Target.Parent then
        local tHum = Shared.Target:FindFirstChildOfClass("Humanoid")
        if tHum and tHum.Health > 0 and IsSmartMatch(Shared.Target.Name, targetMobType) then
            local island = GetNearestIsland(Shared.Target:GetPivot().Position, Shared.Target.Name)
            return Shared.Target, island, "Mob"
        end
    end

    -- Build match list — strict: only exact quest mob type, no boss variants
    local matches = {}
    for _, npc in pairs(PATH.Mobs:GetChildren()) do
        if npc:IsA("Model") and npc:FindFirstChildOfClass("Humanoid") then
            if IsSmartMatch(npc.Name, targetMobType) then
                local cleanName = npc.Name:gsub("%d+$", "")
                -- Extra guard: skip if cleanName contains "Boss" and quest is not a boss quest
                local isBossQuest = targetMobType:lower():find("boss")
                local isBossMob   = cleanName:lower():find("boss")
                if isBossQuest or not isBossMob then
...
local function ExecuteFarmLogic(target, island, farmType)
    ...
    -- SAFE HEIGHT: 45 studs above mob = out of melee reach, M1 still registers
    -- via targetPos arg. Player can increase via Distance slider if needed.
    local SAFE_ABOVE = math.max(distVal, 45)

    if Shared.AltActive then
        finalPos = targetPos + Vector3.new(0, 120, 0)
    elseif posType == "Above" then
        finalPos = targetPos + Vector3.new(0, SAFE_ABOVE, 0)
    elseif posType == "Below" then
        finalPos = targetPos + Vector3.new(0, -distVal, 0)
    else
        -- "Behind" — go above instead so mobs can't reach
        finalPos = targetPos + Vector3.new(0, SAFE_ABOVE, 0)
    end
    ...
    -- FIXED: also attempt M1 here after positioning, passing targetPos
...
local function Func_AutoM1()
    while task.wait(Options.M1Speed.Value) do
        if Toggles.AutoM1.Value then
            -- pass target position so server registers the hit
            local targetPos = nil
            if Shared.Target and Shared.Target.Parent then
                local npcRoot = Shared.Target:FindFirstChild("HumanoidRootPart")
                if npcRoot then targetPos = npcRoot.Position end
            end
            if targetPos then
                Remotes.M1:FireServer(targetPos)
            else
    ...
    while Toggles.AutoInfiniteTower.Value do
        ...
        if isInTower then
            ...
            if bestTarget then
                ...
                local SAFE_ABOVE2 = math.max(distVal, 45)
                if posType == "Above" then
                    finalPos = targetPos + Vector3.new(0, SAFE_ABOVE2, 0)
                    finalCFrame = CFrame.new(finalPos, finalPos + Vector3.new(0, -1, 0))
                elseif posType == "Below" then
                    finalPos = targetPos + Vector3.new(0, -distVal, 0)
                    finalCFrame = CFrame.new(finalPos, finalPos + Vector3.new(0, 1, 0))
                else
                    -- Behind → go safe above instead
                    finalPos = targetPos + Vector3.new(0, SAFE_ABOVE2, 0)
                    finalCFrame = CFrame.new(finalPos, finalPos + Vector3.new(0, -1, 0))
                end
...
local Window = Library:CreateWindow({
	...
	NotifySide = "Right",
    Icon = "rbxassetid://74730846535909",
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().BelleSg_Running then break end
...
task.spawn(function()
    while getgenv().BelleSg_Running do
    ...
    while true do
        ...
        local success, err = pcall(function()
            ...
            if npcHum and npcRoot and root then
                ...
                -- FIXED: removed restrictive <35 dist gate; pass targetPos so server
                -- accepts the hit even when player is above/beside the mob
                local m1Delay = tonumber(Options.M1Speed.Value) or 0.2
                if tick() - Shared.LastM1 >= m1Delay then
...
task.spawn(function()
    while task.wait() do
        ...
        -- Only switch target when mob is fully gone from workspace (Parent == nil)
        -- OR health has been 0 for at least 0.4s — prevents premature target swap
        if Shared.TargetValid and Shared.Target then
            local tgt = Shared.Target
            local isDead = not tgt.Parent
            if not isDead then
                local tHum = tgt:FindFirstChildOfClass("Humanoid")
                isDead = tHum and tHum.Health <= 0
            end
            if isDead then
                -- Confirm dead: wait until actually removed or 0.4s timeout
                if not Shared._killConfirmTick then
                    Shared._killConfirmTick = tick()
                end
                if not tgt.Parent or (tick() - Shared._killConfirmTick >= 0.4) then
                    Shared.KillTick = tick()
                    Shared.TargetValid = false
                    Shared.Target = nil
                    Shared._killConfirmTick = nil
                end
            else
                Shared._killConfirmTick = nil -- still alive, reset
            end
        elseif Shared.TargetValid and not Shared.Target then
            Shared.KillTick = tick()
            Shared.TargetValid = false
            Shared._killConfirmTick = nil
        end
...
ThemeManager:SetFolder("Belle.sg | Sailor Piece")
SaveManager:SetFolder("Belle.sg/SailorPiece")
