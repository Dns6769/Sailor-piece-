...
local function ExecuteFarmLogic(target, island, farmType)
    ...
    if tick() - Shared.LastM1 >= m1Delay then
        -- Force equip any melee/combat tool before firing
        local char3 = GetCharacter()
        if char3 then
            local hasWeapon = char3:FindFirstChildOfClass("Tool")
            if not hasWeapon then
                local combatTool = Plr.Backpack:FindFirstChild("Combat")
                               or Plr.Backpack:FindFirstChildOfClass("Tool")
                if combatTool then
                    local hum3 = char3:FindFirstChildOfClass("Humanoid")
                    if hum3 then hum3:EquipTool(combatTool) end
                else
                    EquipWeapon()
                end
            elseif Toggles.SwitchWeapon.Value then
                EquipWeapon()
            end
        end
        pcall(function()
...
local function Func_AutoM1()
    while task.wait(Options.M1Speed.Value) do
        if Toggles.AutoM1.Value then
            -- FIXED: pass target position so server registers the hit
            local targetPos = nil
    ...
    while true do
        ...
        local success, err = pcall(function()
            ...
            if npcHum and npcRoot and root then
                ...
                local m1Delay = tonumber(Options.M1Speed.Value) or 0.2
                if tick() - Shared.LastM1 >= m1Delay then
                    -- FIXED: always ensure a melee/combat weapon is equipped before M1
                    -- regardless of SwitchWeapon toggle, same as d68 reference behavior
                    local char2 = GetCharacter()
                    if char2 then
                        local hasWeapon = char2:FindFirstChildOfClass("Tool")
                        if not hasWeapon then
                            -- Try equip Combat first, then fallback to EquipWeapon()
                            local combatTool = Plr.Backpack:FindFirstChild("Combat")
                                           or Plr.Backpack:FindFirstChildOfClass("Tool")
                            if combatTool then
                                local hum2 = char2:FindFirstChildOfClass("Humanoid")
                                if hum2 then hum2:EquipTool(combatTool) end
                            else
                                EquipWeapon()
                            end
                        elseif Toggles.SwitchWeapon.Value then
